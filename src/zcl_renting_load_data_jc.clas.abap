CLASS zcl_renting_load_data_jc DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_renting_load_data_jc IMPLEMENTATION.



  METHOD if_oo_adt_classrun~main.

    DATA: lt_brands    TYPE TABLE OF zrent_brands_jc,
          lt_cars      TYPE TABLE OF zrent_cars_jc,
          lt_customers TYPE TABLE OF zrent_custom_jc.
** BRANDS **
    lt_brands = VALUE #(
    ...
    ( marca = 'CUPRA' url ='https://cdn.autobild.es/sites/navi.axelspringer.es/public/bdc/dc/fotos/CUPRA-Ateca-2020_01_HQ_0.jpg?tf=3840x') ).

    DELETE FROM zrent_brands_jc.
    INSERT zrent_brands_jc FROM TABLE @lt_brands.
    SELECT * FROM zrent_brands_jc INTO TABLE @lt_brands.
    out->write( sy-dbcnt ).
...
** CARS**
    lt_cars = VALUE #(
        ...
        ( matricula = 'NJT2227' marca = 'CUPRA' modelo = 'ATECA' color = 'NEGRO' motor = '2.0' potencia = '300' und_potencia = 'CV'
        combustible = 'GASOLINA' consumo = '5.00' fecha_fabr = '20220812' puertas = '5' precio = '180' moneda = 'USD' alquilado = 'X' alq_desde = '20211106' alq_hasta = '20241231' )
    ).

    DELETE FROM zrent_cars_jc.
    INSERT zrent_cars_jc FROM TABLE @lt_cars.
    SELECT * FROM zrent_cars_jc INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
...
** CUSTOMERS**
    lt_customers = VALUE #(
        ...
        ( doc_id = '982367739N' matricula = '1246GSA' nombres = 'Clark' apellidos = 'Allan' )
    ...
    DELETE FROM zrent_custom_jc.
    INSERT zrent_custom_jc FROM TABLE @lt_customers.
    SELECT * FROM zrent_custom_jc INTO TABLE
    @lt_customers.
    out->write( sy-dbcnt ).
